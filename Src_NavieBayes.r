...
Naive_Byess<-function(dat)
{
  index <- sample(nrow(dat), 0.7 * nrow(dat))
  train <- dat[index, ]
  test <- dat[-index,]
  #model <- naiveBayes(train[,2] ~ train[,1], data=train, class.prob=TRUE)
  model <- naiveBayes(Class ~ ., data=train, class.prob=TRUE,probability=TRUE)
  pred <- predict(model, test[,-1], probability=TRUE)
  t <- table(pred, test[,1])
  sum(pred==test[,1])/length(test[,1])
  ypredscore = predict(moedl,test[,-1],type="decision")
  
  prop.table(t,2)
  model1<- svm(Class ~ ., data=train,type="C-classification",probability=TRUE,scale = FALSE)
  pred1 <- predict(model1, test[,-1], probability=TRUE)
  t1 <- table(pred1, test[,1])
  prop.table(t1,2)
  sum(pred1==test[,1])/length(test[,1])
  ypredscore = predict(model1,test[,-1],type="decision")
  #binom.test(t[1,1] + t[2,2], nrow(results), p=0.5)
}



dtm_idf<-function(dataf1)
{
 
  reuters=Corpus(DataframeSource(data.frame(as.character(dataf1)),encoding = "UTF-8"),readerControl = list(language = "en"))
...
#   dtm1 <- as.matrix(dtm_tfxidf)
  ...
  datfdtm=datfdtm[,nchar(colnames(datfdtm))<15]
  datfdtm=datfdtm[,colSums(datfdtm)>3]
  return(datfdtm)
}
...
main<-function()
{
  ...
  setwd("D:/R/Social Media/Naive Byess")
  ...
  dataf[,1]=gsub("no|nor|not|isn't|aren't|wasn't|weren't|hasn't|haven't|hadn't|doesn't|don't|didn't|won't|wouldn't|shan't|shouldn't|can't|cannot|never|couldn't|mustn't (\\w+)", "NEG_\\1", dataf[,1])
  dataf[,1]=gsub("quick (\\w+)", "quick_\\1", dataf[,1])
  dataf[,1]=gsub("deliver (\\w+)", "deliverry_\\1", dataf[,1])
  dataf[,1]=gsub("definit (\\w+)", "definit_\\1", dataf[,1])
