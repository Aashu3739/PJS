	 ...
	 if(length(path[[node_no[k]]])!=1)     #puting conditions should not be the "root" only 
		...
		flag<-0
    		...
    		for (j in 2:length(spath))  	#extracting alert at condition and insert value 
  		{
    		...
    		if((as.integer(grepl(">=",spath[j])))!=0 && (fit$frame[k,"yval"]>=0.75)) #to grep the condition at Yval >=0.75
    		  {
        		str<-strsplit(spath[j],split=">=")
        
        		m<-match(str[[1]][1], colnames(log_data)[2:ncol(log_data)])	#find the index of alert from colnames of log_data
				
				a[m]=fit$frame[k,"yval"] 	#matrix for presence and absence
				
				rl[l,m]<-1								#matrix for rules
...
w_file<-function(MT,folder,f_name)					# writting to a file
{
   ...
   file_name<-paste(".\\Output\\Binary_aggr_op\\",folder,"\\",f_name,".csv", sep= "") 
 ...
 for(i in 2:ncol(log_data))				#formultae for diff. object
{
	...
	{if(i==ncol(log_data))
     ...
     fit <- rpart(form,log_data,maxdepth=2)    # creating  a tree for mximum depth of 2
		...
		file_name<-paste(".\\Output\\Binary_aggr_op\\Binary_tree\\I",i,".bmp", sep= "_")
